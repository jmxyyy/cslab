...
def svm_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = torch.mm(X, W)
    correct_class_score = scores[torch.arange(num_train), y].view(num_train, 1)
    margins = scores - correct_class_score + 1
    margins[torch.arange(num_train), y] = 0
    loss = torch.clamp(margins, min=0).sum() / num_train
    loss += reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    # Replace "pass" statement with your code
    # pass
    masks = (margins > 0).double()
    masks[torch.arange(num_train), y] = -torch.sum(masks, dim=1)
    dW += torch.mm(X.t(), masks) / num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    # pass
    y_pred = torch.mm(X, W).argmax(dim=1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    regularization_strengths = learning_rates
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = X[i] @ W
        scores -= scores.max()
        loss += -scores[y[i]] + scores.exp().sum().log()
        for j in range(num_classes):
            coeff = scores[j].exp() / scores.exp().sum()
            if j == y[i]:
                coeff -= 1
            dW[:, j] += coeff * X[i]
...
def softmax_loss_vectorized(
    ...
    loss = torch.mean(scores.exp().sum(dim=1, keepdim=True).log() - scores[torch.arange(num_train), y].view(num_train, 1))
    loss += reg * torch.sum(W * W)
    
    probs = scores.exp() / scores.exp().sum(dim=1, keepdim=True)
