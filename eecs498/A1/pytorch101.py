...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    e = (stop // 10) * 10
    if s < e:
        x = torch.arange(start=s, end=stop, step=10, dtype=torch.float64)
    else:
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    # pass
    y = torch.zeros(4, 6, dtype=torch.int64)
    y[:2, 1] = 1
    y[:2, 2:] = 2
    y[2:, 0::2] = 3
    y[2:, 1::2] = 4
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    C = 1 + torch.tensor(x).max().item()
    y = torch.zeros(N, C)
    idx0 = torch.arange(N)
    y[idx0, torch.tensor(x)] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    # pass
    x[torch.arange(x.shape[0]), x.argmin(dim=1)] = 0
    y = x.clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sigma = ((x - mu).pow(2).sum(dim=0) / (x.shape[0] - 1)).sqrt()
    y = (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
